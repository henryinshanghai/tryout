...
public class MyThreadPoolStatus_01 {
    ...
    // 与运算 掩码取反(得到左边3位的001) 表示线程池当前处在 STOP状态
    public static int runStateOf(int c) {
        return c & ~COUNT_MASK;
    }

    // 掩码操作，得到右边的29位 aka 工作线程数量
    private static int workerCountOf(int c) {
        return c & COUNT_MASK;
    }

    // 把左边3位 与 右边29位 按照异或运算，合并成为一个值
    ...
    // execute()方法的实现 - 作用：使用者使用execute()方法向线程池提交任务
    public void execute(Runnable command) {
        // 获取线程数量、线程池状态 - 手段：Integer类型
        int c = ctl.get();

        // 如果工作线程 小于 核心线程数量，则：创建新线程并执行任务
        if (workerCountOf(c) < corePoolSize) {
            // addWorker是另一个非常重要的方法
            if (addWorker(command, true)) { // Ⅲ-①
                return;
            }
            // 如果创建失败，则：为防止 外部已经在线程池中加入了新任务，这里重新获取一次
            c = ctl.get();
        }

        // 如果线程池处于 RUNNING状态，则/才会：把新任务添加到队列中
        if (isRunning(c) && workQueue.offer(command)) {
            int recheck = ctl.get();
            // 如果线程池不是 RUNNING的状态，则：把 刚刚加入到队列的任务移除掉
            if (!isRunning(recheck) && remove(command)) {
                reject(command);
            }
            // 如果之前的线程已经被消费完，则：新建一个线程
            else if (workerCountOf(recheck) == 0) {
                addWorker(null, false); // Ⅲ-②
            }
        }
        // 如果：核心线程 与 队列都已经满了的话，尝试创建一个新的线程
        else if (!addWorker(command, false)) { // Ⅲ-③
            // 如果addWorker返回的是 false, 说明创建失败，则：唤醒拒绝策略
            reject(command);
        }
    }
    ...
    /*
        根据当前线程池的状态，检查是否可以添加新的任务线程
            如果可以，则：创建新线程，并启动。返回true
            如果返回false，则表示：1 线程池没有处在 RUNNING的状态； 2 线程工厂创建新的线程失败

        firstTask 从外部启动线程池时，需要创建的第一个线程。
        core：新增工作线程时的判断依据；
            true： 表示新增工作线程时，需要判断当前 RUNNING状态的线程 是不是少于 corePoolSize
            false：表示新增工作线程时，需要判断当前RUNNING状态的线程数量 是不是的号于 maximumPoolSize
     */
