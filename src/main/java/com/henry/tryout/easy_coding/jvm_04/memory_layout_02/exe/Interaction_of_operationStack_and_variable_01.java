package com.henry.tryout.easy_coding.jvm_04.memory_layout_02.exe;

// 验证：方法的执行 底层其实就是 JVM栈区域中 局部变量表 与 操作数栈 之间的交互过程
...
启示：
1 局部变量表 就像是一个 中药柜，有很多 有编号的小盒子。
字节码指令 ISTORE_1 的作用 就是 打开 1号盒子，然后把 栈顶的数字13 存储进去。
栈是一个 很深的竖桶，任何时间都只能够 对 桶口的元素 进行操作 - 因此 数据的存取 只能发生在 栈顶。

特征：有些指令可以直接在 盒子中进行，比如 iinc指令。对盒子中的数值进行+1操作
i++ 与 ++i的区别：
a=i++

0：iload_1 // 从 局部变量表的第一号盒子 中 取出数字，并 压入栈顶
1: iinc 1,1 // 直接 在盒子中 实现 +1的操作
4: istore_2 // 把 栈顶元素 赋值给 a

a=++i
0: iinc 1,1 // 在 局部变量表的1号盒子 中 执行 +1的操作
3: iload_1 把 1号盒子中的数字 压入栈顶
4: istore_2 // 把 +1后的值 赋值给a

特征： i++并不是一个原子操作，
即便 使用 volatile修饰，多个线程 同时写 时，仍旧会出现 数据覆盖的情况
 */