...
// 语法：把 数组 转化成为 list - 手段：Arrays.asList(<array>)
// 验证：转换后得到的list 其实只是 原始数组的一个视图(适配器模式)
// 安全的使用方式：使用 转换得到的list 作为参数，手动new出 一个新的list对象
public class ArrayToList_02 {
    public static void main(String[] args) {
        ...
        // 把 数组 转换成为 list - 手段：Arrays.asList()
        List<String> stringList = Arrays.asList(stringArray);

        /* 对 转换得到的list 尝试各种操作 👇 */
        // ① 修改 list中的元素 - 成功
        stringList.set(0, "oneList");
        // 验证原数组：查看数组中的第一个元素 - 结果：oneList     说明：原始的数组也 随之被改变
        System.out.println(stringArray[0]);

        // ② 向list中 添加元素 - 编译 不会报错，但是 运行会报错 UnsupportedOperationException
//        stringList.add("four");
        // ③ 从list中 移除元素
//        stringList.remove(2);
        // ④ 清除list中的所有元素
//        stringList.clear();
        ...
        /*
            以上，说明: 转换后得到的list 其实只是 原始数组的一个视图(适配器模式)。list本身 存在诸多限制
                原理： Arrays.asList()返回是一个 内部类ArrayList.
        */

        // 数组转集合 安全的方式 - 手段：使用 转换得到的list 作为参数，手动new出 一个新的list对象
...
/*
启示：
    1 Arrays.asList()方法 采用的是 适配器模式 - 后台使用的 仍旧是原有数组；
