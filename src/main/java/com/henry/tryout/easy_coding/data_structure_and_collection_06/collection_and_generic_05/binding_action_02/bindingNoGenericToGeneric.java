...
// 验证：非泛型集合（List / JSONArray）可以赋值给任何 泛型集合 - 但可能会导致 运行时的报错
...
public class bindingNoGenericToGeneric {
    public static void main(String[] args) {
        ...
        if (jsonObject != null) {
            // 这里把 解析出来的 字符串‘3’ 添加到了 intList列表中
            // 特征：编译时不报错，但运行时报错 - java.lang.String cannot be cast to java.lang.Integer
            /*
                原因：
                    1 getJSONArray()方法的 返回值类型 JSONArray；- 这里为什么一定要使用 getJSONArray()返回一个JSONArray呢？
            ...
            int amount = 0;
            for (Integer x : intList) { // 此处运行时报错：java.lang.String cannot be cast to java.lang.Integer
                if (x < 10) {
                    amount = amount + 1;
                }
            }
...
结论：
    ...
    2 由于JDK在5之后 才引入泛型，所以 可能会出现 非泛型对象 赋值给 泛型变量 的代码；
    3 在JDK5之后，应该 尽量使用 泛型 来 定义变量。
 */