...
// 验证：在集合中，能够限定 “元素类型 可以为 多个不同的类型” - 但是 需要遵守 特定的约束
// 手段1：<? extends T> 适用于 get元素的场合；
//      它能够: #1 获取到 T类型的元素; #2 但不支持 任何的add()操作
// 手段2：<? super T> 适用于 存储元素的场合；
//      它能够：#1 添加 T类型 与 <T类型子类>的元素; #2 但是 获取元素 时 只能够得到 Object类型的item
public class multipleTypeWithGenericRestrict {
    public static void main(String[] args) {
        // 目标：希望集合中，能够添加 多种类型的元素(这些类型 都受到 某一个泛型的约束)
        ...
        /*
            ...
            特征：
                1 允许添加 T类型的实例 与 所有 继承自T类型的子类 的实例 进入集合中；
                2 取出item时，item的类型 会被向上强转为 T(因为编译器 只记录了 T类型).
...
//        List<? extends Cat> extendsCatFromCat = animals; // 编译报错， 因为 Animal 不是继承自Cat的
        // 把 List<Animal> 类型的变量 赋值到 List<? super Cat>类型的变量上
        List<? super Cat> superCatFromAnimal = animals; // 编译成功， 因为 Animal super from Cat

        // 把 List<Cat>类型的变量 赋值到 List<? extends Cat>类型的变量上
        List<? extends Cat> extendsCatFromCat = cats; // 编译通过 因为Cat 是继承自Cat的
        // 把 List<Cat>类型的变量 赋值到 List<? super Cat>类型的变量上
        List<? super Cat> superCatFromCat = cats; // 编译通过 因为Cat 是super自Cat的
...
启示：
    List<? super T>的泛型声明 与 所能够添加元素的类型(T与T的子类型) 为什么是矛盾的？
        泛型约束：约束 变量绑定时 所能接受的变量类型;
        add操作：与 泛型约束 没有直接联系，只能添加 T或者T的子类???
 */
