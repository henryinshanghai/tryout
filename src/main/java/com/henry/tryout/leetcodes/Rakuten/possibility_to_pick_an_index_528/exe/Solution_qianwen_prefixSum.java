...
public class Solution_qianwen_prefixSum {
     ...
     */
    public Solution_qianwen_prefixSum(int[] currentSpotToItsWeightArr) {
    ...
    public int pickIndex() {
        ...
        int rightBarCursor = currentSpotToItsPrefixSum.length;

        // 当 左边界指针 不小于 右边界指针 时，说明 此时 左指针与右指针 都已经 指向了“bound位置”，则：退出循环
        while (leftBarCursor < rightBarCursor) { // ① 原则：‘左闭右开区间’的有效性，因此 左边界指针 < 右边界指针(而不是 <=)
            // 计算出 当前区间的中间位置
